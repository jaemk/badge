  ...
  (delay
    {:app-port    (env "PORT" :default 3003 :parse u/parse-int)
     ...
     :max-client-connections-per-host
                  (env "MAX_CLIENT_CONNECTIONS_PER_HOST" :default 50 :parse u/parse-int)
...
(defn build-log-map [data]
  (let [{:keys [level
                ...
                ?line
                ?msg-fmt]} data
        ^Instant real-instant (.toInstant instant)
